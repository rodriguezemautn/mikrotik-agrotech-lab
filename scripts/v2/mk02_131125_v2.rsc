# nov/13/2025 18:14:00 - Rev. 3 - Optimizado QinQ desencapsulación, MTU, WPA3, bridge vlan
# ========================================
# MK02: ISP Magdalena
# IP: 10.200.1.10/24
# User: laboratorio / Lab2025!
# Desencapsula S-VLAN 201 y gestiona AP local.
# Cambios Rev.3: ether-type=0x88a8, MTU=1590, ingress-filtering, tag-stacking=no para desencapsular
# ========================================
...
/interface vlan
add comment="S-VLAN Decapsulation (Q-in-Q)" interface=BR-MAIN name=VLAN201-Trunk vlan-id=201
...
/interface wireless
set wlan1 disabled=no mode=ap-bridge ssid="AgroTech-Magdalena" band=2ghz-b/g/n frequency=2427 channel-width=20mhz security-profile=WiFi-Priv-Profile tx-power=10 country=argentina mtu=1590
add disabled=no master-interface=wlan1 name=wlan-guest ssid="AgroTech-Guest-MG" security-profile=WiFi-Guest-Profile mtu=1590

/interface bridge port
add bridge=BR-MAIN interface=ether2 comment="From MK01" ingress-filtering=yes frame-types=admit-only-vlan-tagged
add bridge=BR-MAIN interface=ether1 comment="To SXT-MG (PtP AP)" ingress-filtering=yes frame-types=admit-only-vlan-tagged
...
add bridge=BR-WiFi-Priv interface=wlan1 pvid=90 ingress-filtering=yes frame-types=admit-only-untagged-and-priority-tagged
add bridge=BR-WiFi-Priv interface=VLAN90-Local

add bridge=BR-WiFi-Guest interface=wlan-guest pvid=96 ingress-filtering=yes frame-types=admit-only-untagged-and-priority-tagged
add bridge=BR-WiFi-Guest interface=VLAN96-Local

/interface bridge vlan
add bridge=BR-MAIN tagged=ether1,ether2,BR-MAIN untagged=VLAN10-Transit vlan-ids=10
add bridge=BR-MAIN tagged=ether1,ether2,BR-MAIN untagged=VLAN20-Transit vlan-ids=20
add bridge=BR-MAIN tagged=ether1,ether2,BR-MAIN untagged=VLAN90-Local vlan-ids=90
add bridge=BR-MAIN tagged=ether1,ether2,BR-MAIN untagged=VLAN96-Local vlan-ids=96
add bridge=BR-MAIN tagged=ether1,ether2,BR-MAIN untagged=VLAN201-Transit vlan-ids=201
add bridge=BR-MAIN tagged=ether1,ether2,BR-MAIN untagged=VLAN201-Trunk vlan-ids=201
...
/ip firewall mangle
add action=change-mss chain=forward new-mss=clamp-to-pmtu passthrough=yes protocol=tcp tcp-flags=syn comment="Clamp MSS"
...
/tool e-mail
set address="smtp.gmail.com" port=587 start-tls=yes from="protocolosinlambrica@gmail.com" user="protocolosinlambrica@gmail.com" password="protocolos.25"  # App password recomendado
