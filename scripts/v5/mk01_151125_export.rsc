# nov/15/2025 01:15:29 by RouterOS 6.49.19
...
set [ find default-name=ether5 ] comment="Local CCTV/IoT" l2mtu=1600 name=\
    ether5-local
/interface vlan
add comment="S-VLAN 4000 - Transporte ISP (Service Tag)" interface=ether2-isp \
    mtu=1590 name=s-vlan-4000 vlan-id=4000
...
add comment="C-VLAN 10 - Servers (encapsulada en S-VLAN 4000)" interface=\
    s-vlan-4000 mtu=1580 name=qinq-vlan10 vlan-id=10
add comment="C-VLAN 20 - Desktop (encapsulada en S-VLAN 4000)" interface=\
    s-vlan-4000 mtu=1580 name=qinq-vlan20 vlan-id=20
add comment="C-VLAN 90 - Private WiFi (encapsulada en S-VLAN 4000)" \
    interface=s-vlan-4000 mtu=1580 name=qinq-vlan90 vlan-id=90
add comment="C-VLAN 96 - Guest WiFi (encapsulada en S-VLAN 4000)" interface=\
    s-vlan-4000 mtu=1580 name=qinq-vlan96 vlan-id=96
add comment="C-VLAN 201 - CCTV (encapsulada en S-VLAN 4000)" interface=\
    s-vlan-4000 mtu=1580 name=qinq-vlan201 vlan-id=201
add comment="C-VLAN 999 - Management (encapsulada en S-VLAN 4000)" interface=\
    s-vlan-4000 mtu=1580 name=qinq-vlan999 vlan-id=999
/interface list
...
add address-pool=pool-vlan10 disabled=no interface=qinq-vlan10 lease-time=1h \
    name=dhcp-qinq-vlan10
add address-pool=pool-vlan20 disabled=no interface=qinq-vlan20 lease-time=1h \
    name=dhcp-qinq-vlan20
add address-pool=pool-vlan90 disabled=no interface=qinq-vlan90 lease-time=8h \
    name=dhcp-qinq-vlan90
add address-pool=pool-vlan96 disabled=no interface=qinq-vlan96 lease-time=1h \
    name=dhcp-qinq-vlan96
add address-pool=pool-vlan201 disabled=no interface=qinq-vlan201 lease-time=\
    1d name=dhcp-qinq-vlan201
...
add bridge=BR-LOCAL tagged=BR-LOCAL,s-vlan-4000 untagged=ether4-local \
    vlan-ids=10
...
add bridge=BR-LOCAL tagged=BR-LOCAL,s-vlan-4000 untagged=ether5-local \
    vlan-ids=201
add bridge=BR-LOCAL tagged=BR-LOCAL,s-vlan-4000 untagged=ether3-mgmt \
    vlan-ids=999
...
add address=192.168.10.1/24 comment="Gateway remoto VLAN 10 via Q-in-Q" \
    interface=qinq-vlan10 network=192.168.10.0
add address=192.168.20.1/24 comment="Gateway remoto VLAN 20 via Q-in-Q" \
    interface=qinq-vlan20 network=192.168.20.0
add address=192.168.90.1/24 comment="Gateway remoto VLAN 90 via Q-in-Q" \
    interface=qinq-vlan90 network=192.168.90.0
add address=192.168.96.1/24 comment="Gateway remoto VLAN 96 via Q-in-Q" \
    interface=qinq-vlan96 network=192.168.96.0
add address=192.168.201.1/24 comment="Gateway remoto VLAN 201 via Q-in-Q" \
    interface=qinq-vlan201 network=192.168.201.0
add address=10.10.10.2/30 comment="WAN IP - Simulacion laboratorio" \
    interface=ether1-wan network=10.10.10.0
add address=10.200.1.1/24 comment="Management IP remoto via Q-in-Q" \
    interface=qinq-vlan999 network=10.200.1.0
/ip dhcp-client
