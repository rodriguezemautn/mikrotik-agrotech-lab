...
# ============================================================================
# ROL: Hub Magdalena - Desencapsulador Q-in-Q y WDS Master
#      - Desencapsulación Q-in-Q (extrae C-VLANs de S-VLAN 4000)
#      - WDS Dynamic Hub hacia SXT-MG
#      - Bridge L2 transparente para VLANs corporativas
#      - AP local para oficina Magdalena
# ============================================================================
...
/interface ethernet
set [ find default-name=ether1 ] name=ether1-spare l2mtu=1600 mtu=1500 comment="Spare port"
set [ find default-name=ether2 ] name=ether2-isp l2mtu=1600 mtu=1590 comment="ISP Q-in-Q Trunk from MK01"
set [ find default-name=ether3 ] name=ether3-mgmt l2mtu=1600 mtu=1500 comment="Management Access"
set [ find default-name=ether4 ] name=ether4-local l2mtu=1600 mtu=1500 comment="Local Trunk"
set [ find default-name=ether5 ] name=ether5-local l2mtu=1600 mtu=1500 comment="Local Trunk"
...
# Bridge para transporte L2 transparente de VLANs
/interface bridge
...
# Estas interfaces VLAN "desencapsulan" el tráfico Q-in-Q
...
# ----------------------------------------------------------------------------
# FASE 5: SEGURIDAD INALÁMBRICA Y WDS
# ----------------------------------------------------------------------------

:log info "MK02: Configurando perfiles wireless..."

# Perfil para enlace WDS hacia SXT-MG (PtP)
/interface wireless security-profiles
add name=wds-link-sxt \
    mode=dynamic-keys \
    authentication-types=wpa2-psk \
    unicast-ciphers=aes-ccm \
    group-ciphers=aes-ccm \
    wpa2-pre-shared-key="Link.AgroTech.WDS.SXT.2025!" \
    comment="WDS Link security to SXT-MG"

# Perfil para AP local (oficina Magdalena)
...
:log info "MK02: Perfiles wireless creados."

# Configuración de WDS Hub (wlan1) - Enlace hacia SXT-MG
/interface wireless
set [ find default-name=wlan1 ] \
    ...
    channel-width=20mhz \
    frequency=2437 \
    ssid="Agrotech-WDS-Hub-MG" \
    security-profile=wds-link-sxt \
    wds-mode=static \
    wds-default-bridge=BR-TRANSPORT \
    wps-mode=disabled \
    country=argentina \
    distance=indoors \
    comment="WDS Hub to SXT-MG"

# IMPORTANTE: WDS static requiere agregar manualmente el peer
# Esto se hace después de que SXT-MG se conecte, con:
/interface wireless wds add master-interface=wlan1 wds-address=E4:8D:8C:82:7C:40

:log info "MK02: WDS Hub configurado."
...
add bridge=BR-TRANSPORT interface=vlan10-extracted comment="C-VLAN 10 desencapsulada"
add bridge=BR-TRANSPORT interface=vlan20-extracted comment="C-VLAN 20 desencapsulada"
add bridge=BR-TRANSPORT interface=vlan90-extracted comment="C-VLAN 90 desencapsulada"
add bridge=BR-TRANSPORT interface=vlan96-extracted comment="C-VLAN 96 desencapsulada"
add bridge=BR-TRANSPORT interface=vlan201-extracted comment="C-VLAN 201 desencapsulada"
...
# ether4/5: Trunk local (todas las VLANs tagged)
add bridge=BR-TRANSPORT interface=ether4-local \
    frame-types=admit-all \
    comment="Local Trunk - All VLANs"

add bridge=BR-TRANSPORT interface=ether5-local \
    frame-types=admit-all \
    comment="Local Trunk - All VLANs"

# wlan1: WDS link (todas las VLANs van por WDS)
add bridge=BR-TRANSPORT interface=wlan1 \
    frame-types=admit-all \
    comment="WDS Link - All VLANs"
...
add bridge=BR-TRANSPORT vlan-ids=10 \
    tagged=BR-TRANSPORT,wlan1,ether4-local,ether5-local
...
:log info "MK02: Configurando DHCP relay..."

# DHCP Relay hacia MK01 (servidor DHCP centralizado)
# En RouterOS 6.x, el DHCP relay debe escuchar en interfaces específicas

# Nota: Como este equipo es L2 bridge, el DHCP pasa transparentemente
# Solo configuramos relay si hubiera IPs locales en VLANs

:log info "MK02: DHCP relay no necesario (bridge L2 transparente)."
...
/ip firewall filter
# INPUT chain - Protección del router
...
# FORWARD chain - Como es L2 bridge, mínimo filtering
...
# Permitir todo el forward (L2 bridge transparente)
add chain=forward \
    action=accept \
...
:log warning "============================================"
:log warning "MK02 - CONFIGURACION COMPLETADA"
:log warning "============================================"
:log warning "Hub Magdalena - Q-in-Q Decapsulator + WDS"
...
:log warning "WDS Hub: wlan1 hacia SXT-MG"
:log warning "============================================"
:log warning "PASOS SIGUIENTES:"
:log warning "1. Conectar ISP trunk en ether2-isp"
:log warning "2. Conectar laptop gestión en ether3-mgmt"
:log warning "3. Cuando SXT-MG se conecte, agregar WDS peer:"
:log warning "   /interface wireless wds add master-interface=wlan1 \\"
:log warning "   wds-address=XX:XX:XX:XX:XX:XX"
:log warning "4. Verificar: /system script run check-qinq-decap"
:log warning "5. Verificar: /system script run check-wds"
:log warning "============================================"
