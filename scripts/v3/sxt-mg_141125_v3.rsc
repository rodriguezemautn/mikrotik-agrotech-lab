# nov/14/2025 - Rev. 4 - SXT-MG: PtP AP (Magdalena)
# ====================================================================
# Dispositivo: sxt-mg
# Rol: AP del enlace PtP de 8km. Bridge L2 transparente con NV2 y MTU 1590.
# Configuracion de Gestion: 10.200.1.50/24 en ether1.
# ====================================================================
...
# Wireless Configuration - NV2 AP
/interface wireless
set [ find default-name=wlan1 ] band=2ghz-b/g/n channel-width=20mhz disabled=no mode=ap-bridge name=wlan-ptp-ap security-profile=ptp-wds-sxt ssid="Agrotech-PTP-2437" wps-mode=disabled frequency=2437 wds-mode=static wds-cost=1 wds-default-bridge=BR-PTP distance=8km country=argentina
/interface wireless set wlan-ptp-ap wireless-protocol=nv2

# Puertos
/interface bridge port
# ether1: Gestion Local (Untagged) y Trunk L2 hacia MK02
add bridge=BR-PTP interface=ether1 pvid=999 comment="Local Mgmt/Trunk"
add bridge=BR-PTP interface=wlan-ptp-ap comment="WDS Link"

# REGLAS DE VLAN FILTERING (Permite el paso de todas las C-VLANs)
/interface bridge vlan
# C-VLANs (10, 20, 90, 96) - Entran y salen etiquetadas por ether1 y wlan-ptp-ap
add bridge=BR-PTP vlan-ids=10,20,90,96 tagged=ether1,wlan-ptp-ap comment="C-VLANs Trunk"
...
/ip address
add address=10.200.1.50/24 interface=ether1 comment="IP de Gestion PtP AP"
/ip route
